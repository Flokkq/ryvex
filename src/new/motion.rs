...
///
/// - `NavigationOnly { nav, count }`: user typed something like `3w` (no operator).
/// - `OperatedNavigation { motion_type, nav, count }`: user typed something like `d3w`, `cw`.
/// - `OperatedRange { motion_type, range, count }`: user typed e.g. `di(`, `y%`, etc.
